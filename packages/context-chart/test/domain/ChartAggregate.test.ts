...
describe('ChartAggregate', () => {
  ...
  describe('archive', () => {
    ...
    it('should fail if chart already archived', async () => {
      const createdEvents = await Effect.runPromise(ChartAggregate.create(command));
      const chart = await Effect.runPromise(ChartAggregate.fromEvents(createdEvents));
      const archivedEvent = await Effect.runPromise(ChartAggregate.archive(chart));
      const archivedChart = await Effect.runPromise(
        ChartAggregate.fromEvents([...createdEvents, ...archivedEvent]),
      );
      const reArchivedEvents = await Effect.runPromiseExit(ChartAggregate.archive(archivedChart));

      expect(Exit.isFailure(reArchivedEvents)).toBe(true);
      if (Exit.isFailure(reArchivedEvents)) {
        const error = Cause.failureOption(reArchivedEvents.cause);
  ...
  describe('fromEvents', () => {
    ...
    it('should fail if first event is not ChartCreated', async () => {
      ...
      const result = await Effect.runPromiseExit(ChartAggregate.fromEvents([archiveEvent]));
